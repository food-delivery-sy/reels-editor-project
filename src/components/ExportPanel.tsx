...
import { Save, FileJson, FileCode, Download } from 'lucide-react';
import { useEditor } from '../lib/editor-context';
import { ReelsConfig } from '../lib/models';

const ExportPanel: React.FC = () => {
  const { project, exportToReelsConfig } = useEditor();
  
  // تصدير كملف JSON
  const exportAsJSON = () => {
    const reelsConfig = exportToReelsConfig();
    const jsonString = JSON.stringify(reelsConfig, null, 2);
    ...
    a.href = url;
    a.download = `${project.reelsConfig.general.title || 'reels-config'}.json`;
  ...
  const exportAsHTML = () => {
    ...
    // إنشاء قالب HTML
    const htmlTemplate = generateHTMLTemplate(reelsConfig);
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `${project.reelsConfig.general.title || 'reels-presentation'}.html`;
    document.body.appendChild(a);
    a.click();
  ...
  // إنشاء قالب HTML
  const generateHTMLTemplate = (reelsConfig: ReelsConfig): string => {
    // تحويل كائن reelsConfig إلى نص
    const reelsConfigString = JSON.stringify(reelsConfig, null, 2);
...
<body>
    ...
    const reelsConfig = ${reelsConfigString};
    ...
    class ConfigLoader {
         ...
         * @returns {Object} الإعدادات العامة
         */
        getGeneralSettings() {
        ...
        /**
         * الحصول على مشهد محدد بواسطة المعرف
         * @param {string} slideId معرف المشهد
         * @returns {Object} بيانات المشهد
         */
        getSlideById(slideId) {
            return this.config.slides.find(slide => slide.id === slideId);
        }
        ...
        applyGeneralSettings() {
            ...
            document.documentElement.dir = general.direction || 'rtl';
            ...
            // تطبيق الخطوط
            if (general.fonts) {
                root.style.setProperty('--heading-font', general.fonts.headingFont);
                root.style.setProperty('--body-font', general.fonts.bodyFont);
            }
        }
    }
    // ===== نهاية قسم محمل التكوين =====
    
    // ===== بداية قسم مولد المشاهد =====
    class SlideGenerator {
        ...
        /**
         * توليد جميع المشاهد من ملف التكوين
         * @param {Array} slidesConfig مصفوفة تكوين المشاهد
         */
        generateSlides(slidesConfig) {
        ...
        /**
         * إنشاء مشهد بناءً على التكوين
         * @param {Object} slideConfig تكوين المشهد
         * @returns {HTMLElement} عنصر المشهد
         */
        createSlide(slideConfig) {
            ...
            switch (slideConfig.type) {
                case 'title':
                    this.generateTitleSlide(content, slideConfig);
                    break;
                case 'list':
                    this.generateListSlide(content, slideConfig);
                    break;
                case 'features':
                    this.generateFeaturesSlide(content, slideConfig);
                    break;
                case 'cta':
                    this.generateCtaSlide(content, slideConfig);
                    break;
                default:
                    console.warn(\`نوع المشهد غير معروف: \${slideConfig.type}\`);
            }
         ...
         * @param {Object} config تكوين المشهد
         */
        generateTitleSlide(container, config) {
         ...
         * @param {Object} config تكوين المشهد
         */
        generateListSlide(container, config) {
            const content = config.content;
         ...
         * @param {Object} config تكوين المشهد
         */
        generateFeaturesSlide(container, config) {
            const content = config.content;
         ...
         * @param {Object} config تكوين المشهد
         */
        generateCtaSlide(container, config) {
            const content = config.content;
    ...
    class AnimationController {
         ...
         * @param {Array} slides مصفوفة المشاهد
         * @param {Object} generalSettings الإعدادات العامة
         */
        initialize(slides, generalSettings) {
            this.slides = slides;
            this.generalSettings = generalSettings;
            
            // تهيئة GSAP إذا كان متاحاً
            if (window.gsap) {
                gsap.registerPlugin(ScrollToPlugin);
            } else {
                console.warn('مكتبة GSAP غير متاحة. سيتم استخدام الحركات البسيطة فقط.');
            }
            ...
            // بدء التشغيل التلقائي إذا كان مفعلاً
            if (generalSettings.timing && generalSettings.timing.autoPlay) {
                this.startAutoPlay();
            }
        }
        ...
        setupEventListeners() {
            ...
            // إضافة مستمع لمفاتيح الأسهم
            document.addEventListener('keydown', (event) => {
                if (event.key === 'ArrowLeft') {
                    this.nextSlide(); // للغة العربية، السهم اليسار يعني التالي
                } else if (event.key === 'ArrowRight') {
                    this.prevSlide(); // للغة العربية، السهم اليمين يعني السابق
                }
            });
        }
        ...
        showSlide(index) {
            ...
            this.slides.forEach((slide, i) => {
                ...
                if (i === index) {
                    // عرض المشهد الحالي مع تأثير الدخول
                    slideElement.style.display = 'flex';
                    this.applyAnimation(slideElement, slide.config.timing?.transitionIn || 'fadeIn');
                } else {
        ...
        /**
         * تطبيق تأثير حركي على عنصر
         * @param {HTMLElement} element العنصر
         * @param {string} animationType نوع التأثير
         */
        applyAnimation(element, animationType) {
            // إزالة جميع فئات التأثيرات السابقة
            element.classList.remove('fadeIn', 'fadeOut', 'slideInRight', 'slideOutLeft', 'zoomIn', 'zoomOut');
            
            // إضافة فئة التأثير الجديد
            element.classList.add(animationType);
            
            // استخدام GSAP إذا كان متاحاً للحركات المتقدمة
            if (window.gsap) {
                switch (animationType) {
                    case 'fadeIn':
                        ...
                        break;
                    case 'fadeOut':
                        gsap.fromTo(element, { opacity: 1 }, { opacity: 0, duration: 0.8 });
                        break;
                    case 'slideInRight':
                        gsap.fromTo(element, { x: 100, opacity: 0 }, { x: 0, opacity: 1, duration: 0.8 });
                        break;
                    case 'slideOutLeft':
                        gsap.fromTo(element, { x: 0, opacity: 1 }, { x: -100, opacity: 0, duration: 0.8 });
                        break;
                    case 'zoomIn':
                        gsap.fromTo(element, { scale: 0.5, opacity: 0 }, { scale: 1, opacity: 1, duration: 0.8 });
                        break;
                    case 'zoomOut':
                        gsap.fromTo(element, { scale: 1, opacity: 1 }, { scale: 1.5, opacity: 0, duration: 0.8 });
                        break;
                }
                
                // تطبيق حركات على العناصر الداخلية
                this.animateSlideContent(element);
            }
        }

        /**
         * تحريك محتوى المشهد بشكل متتالي
         * @param {HTMLElement} slideElement عنصر المشهد
         */
        animateSlideContent(slideElement) {
            if (!window.gsap) return;
            
            // تحديد العناصر الداخلية للمشهد
            const elements = slideElement.querySelectorAll('.content > *');
            
            // تطبيق حركة متتالية على كل عنصر
        ...
        /**
         * إعادة ضبط مؤقت التشغيل التلقائي
         */
        resetAutoPlayTimer() {
            if (this.autoPlayInterval) {
                clearInterval(this.autoPlayInterval);
            ...
            // الحصول على مدة المشهد الحالي أو استخدام المدة الافتراضية
            const currentSlide = this.slides[this.currentSlideIndex];
            const duration = currentSlide.config.timing?.duration || 
                            this.generalSettings.timing?.defaultSlideDuration || 
                            5000;
    ...
    // ===== بداية قسم الوظائف المساعدة =====
    /**
     * تحميل رموز QR للروابط
     */
    function loadQRCodes() {
        // البحث عن جميع عناصر رمز QR
        const qrElements = document.querySelectorAll('.qr-code');
        
        // إذا كانت مكتبة QRCode متاحة، قم بإنشاء رموز QR
        if (qrElements.length > 0) {
            if (window.QRCode) {
                ...
                console.warn('مكتبة QRCode غير متاحة. سيتم عرض النص البديل.');
            }
        }
    }
    // ===== نهاية قسم الوظائف المساعدة =====
    ...
    document.addEventListener('DOMContentLoaded', () => {
        try {
            // تهيئة محمل التكوين
            const configLoader = new ConfigLoader(reelsConfig);
            
            // تطبيق الإعدادات العامة
            configLoader.applyGeneralSettings();
            
            // تهيئة مولد المشاهد
            const slideGenerator = new SlideGenerator(document.getElementById('app'));
            
            // توليد المشاهد
            const slides = slideGenerator.generateSlides(configLoader.getSlides());
            
            // تهيئة وحدة التحكم بالحركة
            const animationController = new AnimationController();
            animationController.initialize(slides, configLoader.getGeneralSettings());
            
            console.log('تم تهيئة العرض التقديمي بنجاح!');
            
            // إضافة وحدة التحكم بالحركة إلى النافذة للوصول إليها من وحدة التسجيل
            window.animationController = animationController;
            
            // تحميل رمز QR إذا كان مطلوباً
            loadQRCodes();
  ...
  return (
    <div className="p-4 border border-border rounded-md bg-card">
      <div className="text-center mb-4">
        <h3 className="font-semibold text-lg mb-1">تصدير المشروع</h3>
        <p className="text-sm text-muted-foreground">
          قم بتصدير المشروع كملف JSON أو HTML
        </p>
      </div>
      ...
      <div className="flex flex-col gap-4">
        ...
        >
          ...
          <span>تصدير كفيديو</span>
        </Button>
        
        <div className="text-xs text-muted-foreground text-center mt-2">
          ملاحظة: تصدير الفيديو سيكون متاحاً في الإصدارات القادمة
        </div>
      </div>
    </div>
  )
